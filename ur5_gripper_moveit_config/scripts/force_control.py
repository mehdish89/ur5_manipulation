...
def get_pose():
	if not listener.canTransform("base", "tool0", rospy.Time(0)):
		return np.array([0.] * 7)
...
def speedl(twist, a = 10, t_min = 0.1):
	pub.publish("speedl([{0},{1},{2},{3},{4},{5}], {6}, {7})".format( twist[0],
																	 twist[1],
																	 twist[2], 
																	 twist[3], 
																	 twist[4], 
...
def movej(joints):
	pub.publish("movej([{0},{1},{2},{3},{4},{5}])".format( joints[0],
																	 joints[1],
																	 joints[2], 
																	 joints[3], 
																	 joints[4], 
...
def servoc(pose, a = .2, v = 0.005, r = 0):
	pub.publish("movel(p[{0},{1},{2},{3},{4},{5}], a = {6}, v = {7}, r = {8})".format(pose[0],
																					 pose[1],
																					 pose[2], 
																					 pose[3]*0, 
																					 pose[4]*0, 
...
for i in range(3):
	...
	graph3, = axarr[2].plot(x_data, y_data/200.)
	plts_force += [graph1]
	plts_torque += [graph2]
	plts_vel += [graph3]
...
def eval_contact():
	...
	while not rospy.is_shutdown():
		if listener.canTransform("base", "tool0", rospy.Time(0)):
			theta = (theta + 0.5) % (2 * math.pi)
			
			wiggle(theta, t_min = 0.25)
			rospy.sleep(0.25)

			ft = read_ft()
			print(sum(abs(ft[0:3])), sum(abs(ft[3:6])))
...
while not rospy.is_shutdown():
	...
	y_new = (sum(abs(ft[0:3])) + sum(abs(ft[3:6])) * trscale * 0)/200
	y_data = np.append(y_data[1:len(y_data)],y_new)
	plts_vel[2].set_xdata(x_data)
	plts_vel[2].set_ydata(y_data)

	# print(sum(y_data)/100.)
